...
if ( ! function_exists('ci_create_captcha'))
{
	/**
	 * Create CAPTCHA
	 *
	 ...
	 */
	function ci_create_captcha($data = '', $img_path = '', $img_url = '', $font_path = '')
	{
		$defaults = [
			'word'		=> '',
			'img_path'	=> '',
		...
		if (empty($word))
		{
			...
			if (($bytes = random_bytes($pool_length)) !== FALSE)
			{
		...
		foreach (array_keys($defaultColor) as $key)
		{
